import sys; sys.path.insert(0, r'C:\Users\hp\zoya3')
from zoya.cloud.database import DatabaseService, QueryFilter, QueryOptions, QueryOrder, CollectionSchema, DatabaseError
from zoya.cloud.auth import AuthService, AuthConfig, AuthError
...
r = db5.query('users', QueryOptions(filters=[QueryFilter(field='age', operator='in', value=[25, 30])]))
assert r.total == 2
print('in: OK')

r = db5.query('users', QueryOptions(filters=[QueryFilter(field='name', operator='contains', value='Ali')]))
assert r.total == 1
print('contains: OK')

r = db5.query('users', QueryOptions(filters=[QueryFilter(field='name', operator='startsWith', value='A')]))
assert r.total == 1
print('startsWith: OK')

r = db5.query('users', QueryOptions(filters=[QueryFilter(field='name', operator='endsWith', value='ce')]))
assert r.total == 1
print('endsWith: OK')

r = db5.query('users', QueryOptions(filters=[QueryFilter(field='age', operator='>', value=25)]))
assert r.total == 1
print('>: OK')

r = db5.query('users', QueryOptions(filters=[QueryFilter(field='age', operator='<', value=30)]))
assert r.total == 1
print('<: OK')
