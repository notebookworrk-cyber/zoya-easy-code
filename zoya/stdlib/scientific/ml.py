...
def mse_loss(y_true: Vector, y_pred: Vector) -> float:
    """Mean squared error."""
    return sum((yt - yp) ** 2 for yt, yp in zip(y_true, y_pred, strict=False)) / len(
        y_true
    )
...
class LinearRegression:
    ...
    def score(self, X: Matrix, y: Vector) -> float:
        ...
        y_mean = sum(y) / len(y)
        ss_res = sum((yt - yp) ** 2 for yt, yp in zip(y, y_pred, strict=False))
...
class LassoRegression:
    ...
    def fit(self, X: Matrix, y: Vector) -> "LassoRegression":
        ...
        _n_samples, n_features = len(X), len(X[0])
        w = [0.0] * n_features
        ...
        for _ in range(self.max_iter):
            ...
            for j in range(n_features):
                ...
                if self.fit_intercept and j == 0:
                    w[j] = XTy[j] - sum(
                        XTX[j][k] * w[k] for k in range(n_features) if k != j
                    )
                    w[j] /= XTX[j][j]
...
class LogisticRegression:
    ...
    def score(self, X: Matrix, y: Vector) -> float:
        ...
        return sum(1 for p, t in zip(preds, y, strict=False) if p == t) / len(y)
...
class KMeans:
    ...
    def __init__(
        ...
        self.cluster_centers_: Matrix | None = None
        self.labels_: Vector | None = None
        self._fitted = False
        ...
        for _ in range(self.max_iter):
            ...
            for x in X:
                best = min(
                    ...
                    key=lambda k: self._distance(x, centroids[k]),
                )
                labels.append(best)
            ...
            # Check convergence
            if all(
                self._distance(centroids[i], new_centroids[i]) < self.tol
    ...
    def predict(self, X: Matrix) -> Vector:
        ...
        return [
            min(
                range(self.n_clusters),
                key=lambda k: self._distance(x, self.cluster_centers_[k]),
            )
            for x in X
        ]
...
class GaussianNB:
    ...
    def fit(self, X: Matrix, y: Vector) -> "GaussianNB":
        ...
        for i, c in enumerate(self.classes_):
            X_c = [row for row, label in zip(X, y, strict=False) if label == c]
            ...
            for j in range(n_features):
                ...
                self.var_[i][j] = (
                    sum((x - self.theta_[i][j]) ** 2 for x in feat) / len(feat)
                    + self.var_smoothing
                )
    ...
    def _build_tree(self, X: Matrix, y: Vector, depth: int) -> dict:
        ...
        return {
            ...
            "left": self._build_tree(
                [X[i] for i in left_idx], [y[i] for i in left_idx], depth + 1
            ),
            "right": self._build_tree(
                [X[i] for i in right_idx], [y[i] for i in right_idx], depth + 1
            ),
        }
...
class RandomForestClassifier:
    ...
    def predict(self, X: Matrix) -> Vector:
        ...
        preds = [
            [
                tree.predict([[row[j] for j in feat_idx] for row in X])[i]
...
def confusion_matrix(
    y_true: Vector, y_pred: Vector, labels: list | None = None
) -> Matrix:
    if labels is None:
