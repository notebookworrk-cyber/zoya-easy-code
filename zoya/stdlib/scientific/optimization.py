...
def gradient_descent(
    ...
    for _i in range(max_iter):
        ...
        if momentum > 0:
            v = [momentum * v_i - lr * g_i for v_i, g_i in zip(v, g, strict=False)]
            x = [x_i + v_i for x_i, v_i in zip(x, v, strict=False)]
        else:
            x = [x_i - lr * g_i for x_i, g_i in zip(x, g, strict=False)]
...
def stochastic_gradient_descent(
    ...
    x = x0[:]
    len(x0)  # Assuming data dimension
...
def line_search(
    ...
    fx = f(x)
    g = grad_f(x)
    gtd = sum(gi * di for gi, di in zip(g, d, strict=False))
    ...
    for _ in range(max_iter):
        ...
        g_new = grad_f(x_new)
        gtd_new = sum(gi * di for gi, di in zip(g_new, d, strict=False))

        # Curvature condition
        if gtd_new < c2 * gtd:
            alpha_min = alpha
            alpha = 2 * alpha if alpha_max == 1.0 else (alpha_min + alpha_max) / 2
            continue
...
def conjugate_gradient(
    ...
    for _ in range(max_iter):
        ...
        beta = (
            sum(gi**2 for gi in g_new) / sum(gi**2 for gi in g)
            if sum(gi**2 for gi in g) > 0
            else 0
        )
    ...
    for _ in range(max_iter):
        ...
        if fr >= f_vals[-2]:
            if fr < f_vals[-1]:
                ...
                # Inside contraction
                xc = [
                    centroid[i] - rho * (centroid[i] - simplex[-1][i]) for i in range(n)
                ]
                fc = f(xc)
                if fc < f_vals[-1]:
                    simplex[-1] = xc
