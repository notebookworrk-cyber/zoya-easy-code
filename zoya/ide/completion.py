...
ZOYA_KEYWORDS: List[str] = [
    ...
    "return", "break", "continue", "print", "input", "import", "from",
    "class", "interface", "enum", "match", "switch", "case", "default",
...
STRING_METHODS: List[str] = ["upper", "lower", "strip", "replace", "split", "startswith", "endswith", "contains"]
LIST_METHODS: List[str] = ["append", "remove", "pop", "clear", "insert", "sort", "reverse", "length", "copy"]
...
class CompletionEngine:
    def __init__(self, provider: Optional[LLMProvider] = None):
        self._provider = provider or MockProvider(responses={
            "complete": "suggestion",
        })
    ...
    def get_completions(self, context: CompletionContext) -> List[CompletionItem]:
        ...
        if prefix:
            results = [r for r in results if r.label.startswith(prefix) or r.label.lower().startswith(prefix.lower())]
    ...
    def _get_scope_completions(self, source: str, scope: List[str]) -> List[CompletionItem]:
        ...
        for name, kind in variables.items():
            results.append(CompletionItem(
                label=name,
                kind=kind,
                detail=f"Defined in current scope",
                insert_text=name,
            ))
        return results
    ...
    def _get_dot_completions(self, source: str, line: int, col: int) -> List[CompletionItem]:
        ...
        elif var_type in ("int", "float", "number"):
            methods = []
        elif var_type == "class":
            methods = []
        else:
    ...
    def _get_ai_completions(self, context: CompletionContext) -> List[CompletionItem]:
        try:
            ...
            response = self._provider.chat([
                {"role": "system", "content": "You are a Zoya code completion engine. Return only JSON arrays."},
